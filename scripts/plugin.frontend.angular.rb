...
class PluginFrontendAngular
    ...
    def install
        if !File.directory?(ANGULAR_DEST_FOLDER)
            Rake.mkdir_p(ANGULAR_DEST_FOLDER, verbose: true)
            puts "Creating temporary folder: #{ANGULAR_DEST_FOLDER}"
        end
        if !File.directory?("#{ANGULAR_DEST_FOLDER}/#{PLUGIN_FRONTEND_ANGULAR_BASE_NAME}")
    ...
    def get_dist_path
        return "#{ANGULAR_DEST_FOLDER}/#{PLUGIN_FRONTEND_ANGULAR_BASE_NAME}/dist/#{@package_json['name']}"
    end
